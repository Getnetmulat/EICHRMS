...
$(add_button).click(function(e) {
    ...
    if (x < max_fields) {
        x++;
        $(wrapper).append('<div><input type="text" name="newFieldText[]" required/><a href="#" class="btn btn-danger delete">x</a></div>'); //add input box
    } else {
        alert('ይቅርታ! ከ10 በላይ መጨመር አይቻልም።')
    }
});
...
function topNavFunction() {
  var x = document.getElementById("topN");
...
/*<!-- JS for Updating Drop down menu --> */

$(document).ready(function() {

  $('#all_departments').change(function(){

    $.getJSON('/_update_service_dropdown', {
      selected_dept: $('#all_departments').val()

    }).success(function(data) {
          $('#all_services').html(data.html_service_selected);
      })
  });
  $('#all_departments').change(function(){

    $.getJSON('/_update_employee_dropdown', {
      selected_dept: $('#all_departments').val()

    }).success(function(data) {
          $('#all_employees').html(data.html_employee_selected);
      })
  });

  /* get all weredas under selected zone */
  $('#all_zones').change(function(){

    $.getJSON('/_update_wereda_dropdown', {
      selected_zone: $('#all_zones').val()

    }).success(function(data) {
          $('#all_weredas').html(data.html_wereda_selected);
      })
  });

  /* get all positions under selected department */
  $('#departments').change(function(){

    $.getJSON('/_update_department_position', {
      selected_department_: $('#departments').val()

    }).success(function(data) {
          $('#positions').html(data.html_position_selected);
      })
  });
...
/*<!--simple date picker jquery--> */
    ...
    $(function () {
        $("#date_picker").datepicker({
            ...
            monthNamesShort: ['ጥር','የካቲት','መጋቢት','ሚዚያ','ግንቦት','ሰኔ','ሐምሌ','ነሐሴ','መስከረም','ጥቅምት','ህዳር','ታህሳስ'],
            minDate:0,
            showAnim:'fold',
            dateFormat:'yy-mm-dd'
        });
...
/* Sort table by click table header */
...
function createPDF() {
  ...
  win.document.write('<title>ሪፖርት</title>');   // <title> FOR PDF HEADER.
